...
src/
  ...
  infra/
    config/
      ...
      sentry-config.ts        # SentryConfig interface + loadSentryConfig
      sentry-init.ts          # initSentry(config) — pure Effect
      sentry-capture.ts       # captureEffectError(cause) — sends to Sentry
      tracing-layer.ts        # createTracingLayer — returns NodeSdk layer
      slide-state.ts          # createSlideStatePlugin (Elysia decorator)
    http/
...
All span names are centralized in `src/shared/constants.ts` (`SPANS` map).

| Span | Source | Description |
|------|--------|-------------|
| `ws.message` | `ws-pipeline.ts` | WebSocket message handling |
...
| `agent.tool.<name>` | `agent-tool-factory.ts` | AI tool execution |
| `SlideService.*` | `slide-service-live.ts` | Auto-generated by `Effect.fn` |

## Span Annotations

All attribute keys are centralized in `src/shared/constants.ts` (`ATTRS` map).

| Attribute | Used In | Description |
|-----------|---------|-------------|
| `ws.event_type` | `ws-pipeline.ts` | Message type (JOIN_ROOM, SLIDE_CHANGE, etc.) |
| `deck.id` | `ws-pipeline.ts`, `agent-controller.ts`, `agent-tool-factory.ts` | Deck identifier |
| `client.count` | `effect-runtime.ts` | Number of clients receiving broadcast |
| `ai.model` | `agent-controller.ts` | AI model name (configurable via `AI_MODEL` env var) |
| `tool.name` | `agent-tool-factory.ts` | Tool name |
| `slide.index` | `agent-tool-factory.ts` | Target slide index |
| `slide.from_index` | `agent-tool-factory.ts` | Reorder source index |
| `slide.to_index` | `agent-tool-factory.ts` | Reorder destination index |
| `message.count` | `agent-controller.ts` | Number of chat messages in request |
...
- Called automatically from WS handler (`slide-ws.ts`) and agent tool executor (`agent-tool-executor.ts`) on failure

Domain error → WS message mapping is centralized in `src/shared/errors.ts`:
